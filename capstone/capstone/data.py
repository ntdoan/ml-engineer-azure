...
DEFAULT_PROCESSED_DATA_DIR = (
    PROCESSED_DATA_DIR.joinpath("resolution=5min")
)
...
def preprocess(df: pd.DataFrame, select_columns: List[str], resample_to: str="15min") -> pd.DataFrame: 
    """[summary]
...
def get_data_from_file(file_name=str) -> pd.DataFrame:
    ...
    data_test = np.concatenate((W_test, X_s_test, X_v_test, T_test, Y_test, A_test), axis=1)
...
def output_payload(df: pd.DataFrame, n_samples: int=2, file_path: str="payload.json") -> bool: 
...
def get_file_path(target_resolution: str, file_name: str) -> Path: 
    ...
    """
    out_dir = PROCESSED_DATA_DIR.joinpath(
        "resolution={}".format(target_resolution)
    )
...
if __name__ == "__main__":
    ...
    df_train_raw, df_test_raw, var_names, select_columns = get_data_from_file(file_name="N-CMAPSS_DS02-006.h5")
    df_train_resampled = preprocess(df=df_train_raw, select_columns=select_columns, resample_to=target_resolution)
    df_test_resampled = preprocess(df=df_test_raw, select_columns=select_columns, resample_to=target_resolution)
    ...
    with open(get_file_path(target_resolution=target_resolution, file_name="variable_names.json"), "w") as f:
